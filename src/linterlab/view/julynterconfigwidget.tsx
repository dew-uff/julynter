...
import { ViewMode, IJulynterLintOptions, ErrorTypeKey, IErrorMessage, ReportId, ReportIds, ErrorTypeKeys, ViewModes } from '../../linter/interfaces';
import { ERRORS, ERROR_TYPES_MAP } from '../../linter/errors';
import { MainAreaWidget, Toolbar, ReactWidget, ToolbarButtonComponent, showDialog, Dialog, showErrorMessage } from '@jupyterlab/apputils';
...
class ReportGroup extends React.Component<IReportGroup, IReportGroupState> {
  ...
  clickItem(key: ReportId): () => void {
    const self = this;
    return () => {
      self.props.current.reports[key] = !self.props.current.reports[key];
      self.forceUpdate();
    };
  }
  ...
  render(): JSX.Element {
    ...
    const items = this.props.items.map(el => {
      ...
      return <tr key={key} onClick={this.clickItem(el.key)}>
          <td><input type='checkbox' onChange={this.changeItem} checked={this.props.current.reports[el.key]}/></td>
          <td>{el.key}</td>
    ...
    if (!this.props.current.types[this.props.group]) {
      notice = <span> This category is disabled. The individual options in the following table will have no effect.</span>
    }
    ...
    const error_type = ERROR_TYPES_MAP[this.props.group];
    return <div className='julynter-config-group'>
      <h2>
        <div className='julynter-config-group-head' onClick={this.collapseGroup.bind(this)}>
          {twistButton}
          <span>{error_type.label}</span>
        </div>
        <input type='checkbox' onChange={this.changeGroup.bind(this)} checked={this.props.current.types[this.props.group]}/>
      </h2>
      {children}
    </div>
  }
...
export class JulynterConfigContent extends Widget {
  ...
  display(): void {
    ...
    const reportGroups = ErrorTypeKeys.map(el => {
      ...
      return <ReportGroup current={this._current} key={key} group={el} items={groups[el]}/>
    })
    ...
    renderedJSX = (
      <div className='julynter-config-inner'>
        ...
        <div className='julynter-config-notice'> Do not forget to click the "Save" button when you are done </div>
        ...
        <label>
          ...
          <div className='julynter-config-input' key={'req-' + this._key}>
            <input type='text' onChange={this.writeRequirements.bind(this)} defaultValue={this._current.requirements}/>
          </div>
        </label>
        {reportGroups}
     </div>
...
export class ContextSwitcher extends ReactWidget {
  ...
  constructor(tracker: INotebookTracker, selected: string, content: JulynterConfigContent) {
    super();
  ...
  render(): JSX.Element {
    ...
    this._tracker.forEach((notebook) => {
      let key = 'notebook-save-'+(i++);
      result.push(<option key={key} value={notebook.id}>{notebook.title.label} Settings</option>);
    })
...
export class SaveButton extends ReactWidget {
  ...
  onClick(): void {
    ...
    if (selected === 'user'){
      this._config.saveUser(this._checks).then(()=> {
        showDialog({
          title: 'Success',
          body: <div><div>Stored settings for user.</div><div>It will not change existing notebooks that have their own settings.</div></div>,
      ...
      this._config.saveProject(this._checks).then(()=> {
        showDialog({
          title: 'Success',
          body: <div><div>Stored settings for project.</div><div>It will not change existing notebooks that have their own settings.</div></div>,
          buttons: [ Dialog.okButton({ label: 'Ok' })]
        })
      })
      .catch((reason) => {
      ...
      this._handlers[selected].then((handler) => {
        ...
        showDialog({
          title: 'Success',
          body: <div><div>Stored settings for {handler.name}.</div></div>,
          buttons: [ Dialog.okButton({ label: 'Ok' })]
        })
      });
    } else {
...
export class LoadButton extends ReactWidget {
  ...
  onClick(): void {
    const selected = this._switcher.selected;
    if (selected === 'user'){
      this._config.loadUser().then((config) => {
      ...
      this._config.loadProject().then((config) => {
        this._config.merge(this._checks, config);
        this._content.externalUpdate();
      }).catch((reason) => {
        showErrorMessage('Failed', reason);
...
export function createJulynterConfigWidget(
  ...
  const loadButton = new LoadButton(saveSwitcher, handlers, config, checks, content);
